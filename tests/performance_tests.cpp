...
int main() {
    ...
    passed &= within("add_5000_vector_strokes", milliseconds(start), 2500.0);

    start = Clock::now();
    for (int index = 0; index < 250; ++index) {
        document.erase_at({5000.0F + static_cast<float>(index), 5000.0F}, 2.0F);
    }
    passed &= within("250_eraser_misses_over_5000_objects", milliseconds(start), 1500.0);

    start = Clock::now();
    document.clear();
    document.undo();
    passed &= within("clear_and_restore_5000_objects", milliseconds(start), 2500.0);
    ...
    passed &= within("simplify_100000_samples", milliseconds(start), 1500.0);
